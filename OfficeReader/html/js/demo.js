    ...
    Bing.demo.prototype = {
        isNeedFile : function(arg){//文件格式检测
            ...
            if(arg.needFiles){
                ...
                var needFiles = ',' + arg.needFiles + ',';
                return fileType && needFiles.indexOf(',' + fileType + ',') > -1 ? true : false;
            }
            return true;
        },
        ...
        displayPages : function(screenH,defaultH,screenTop){
            var topPage = Math.floor(screenTop/defaultH);
            var endPage = Math.ceil((screenH + screenTop)/defaultH)-1;
        ...
        parseURL:function(url){
    		...
    		if(arr_url && arr_url[1]){
    			...
    			while((result=reg_para.exec(str_para))!=null){//循环url，直到检查到最后返回为空，停止检测
    				ret[result[1]]=result[2];
    			}
    		}
    		return ret;
    	},
        pasePages : function(obj){
            var total = obj.total,
                currentPage = obj.currentPage,
                size = obj.size,
                start,
                end;
            start = currentPage - size / 2;
            if(start < 1){start = 1}
            end = start + size -1;

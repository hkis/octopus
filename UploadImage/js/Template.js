...
(function(){
    Bing.UpImg = function(arg){
        ...
        this.nameSpace = $('<div class="'+this.arg.className+'"></div>');
        this.fileEle;//file元素
        this.imgContent;//上传的图片显示的区域
        this.submitButton;//图片提交按钮
        this.loading;
        this.iframe;
        this._init();
    };
    Bing.UpImg.prototype = {
        _init : function(){
            var iframe = $('#iframe_hidden');
            this.iframe = iframe;
            if(iframe.length == 0){
               $('body').append('<iframe name="ifr" id="iframe_hidden" style="display:none"></iframe>');
            }
            this.nameSpace.append('<form enctype="multipart/form-data" action="'+this.arg.postUrl+'" target="ifr" method="post" ></form>');
            this.nameSpace.find('form').append('<input type="hidden" name="uid" value="'+this.arg.uid+'" /><input type="hidden" name="action" value="upload" /><label id="action">请上传格式为'+this.arg.allowTypes+'的图片</label>');
            
            this.fileEle = $('<input type="file" name="filedata" />');
            this.imgContent = $('<div id="preview"></div>');
            this.submitButton = $('<input type="button" value="提交" id="submit" style="display:none" />');
            this.loading = $('<span id="loading" style="display:none;">图片上传中</span>');
           
            this.nameSpace.find('form').append(this.fileEle);
            this.nameSpace.append(this.imgContent);
            this.nameSpace.append(this.submitButton);
            this.nameSpace.append(this.loading);
            var $this = this;
            this.fileEle.change(function(){
                var $$this = $this;
        ...
        loadFun : function(){
            var $this = this,
                timer = setInterval(auto,25);
            function auto(){
                var data = Bing.imgData;
                if(!data){return;}//这里不好，imgData是全局变量
                clearInterval(timer);
                data = $.parseJSON(data);
                if(data){
                    if(data.result){
                        var width = data.width,height = data.height,parEle = $this.imgContent;
                        ...
                        imgSpecification.cutPro = $this.arg.imgPro;
                        imgSpecification.parEle = parEle;
                        //添加截图框div
                ...
                $this.loading.html('上传成功').fadeOut('normal');
                delete Bing.imgData;
             }
        },
        createCutDiv : function(arg){
            arg = $.extend({parEle : '',cutPro:0.9,width:200,height:300,left:0,top:0},arg);
            arg.zoom = 0.8;
            ...
            if(parEle){
                ...
                this.submitButton.one('click',function(){
                    $this.submitCutData({parEle:parEle.find('img'),cutDiv:cutDiv,url:uploadUrlAshx,uid:$this.arg.uid,fu:function(){$this.submitButton.fadeOut();}});
                });
                new Bing.Drag({pro:$this.arg.imgPro,moveBar:cutDiv,imgEle:parEle.find('img'),parEle:$this.imgContent,moveBarIds:'top left bottom right top_left top_right bottom_left bottom_right'});
                new Bing.Scroll({eventEle:[parEle.find('img'),cutDiv],parEle:$this.imgContent});
            }else{
    ...
    Bing.Drag = function(arg){
        ...
        (function($$this){
            var $this = $$this;
            if(!$this.arg.moveBar){
                throw new Error('传入参数出错，必须指定moveBar参数');
                return false;
            }
            ...
            elementToDrag.bind('mousedown',function(e){
                ...
                var offset = $($this.img).getParentOffset();//获取元素相对于浏览器窗口的坐标
                
                $this.startLeft = parseFloat(elementToDrag.css('left'));
                $this.startTop = parseFloat(elementToDrag.css('top'));
